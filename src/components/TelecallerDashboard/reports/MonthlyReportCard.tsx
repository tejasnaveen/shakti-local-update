...
export const MonthlyReportCard: React.FC<MonthlyReportCardProps> = ({ data }) => {
  const maxCalls = Math.max(...data.monthlyTrend, 1);
  const hasData = data.monthlyTrend.length > 0 && maxCalls > 0;
  const trendLength = Math.max(data.monthlyTrend.length - 1, 1);
        ...
        <div className="h-20 bg-violet-50 rounded-lg p-3 relative">
          ...
          {hasData ? (
            <svg className="w-full h-full relative z-10" viewBox="0 0 100 40">
              ...
              <path
                d={`M0,${40 - ((data.monthlyTrend[0] || 0) / maxCalls) * 30} ${data.monthlyTrend.map((value, index) => {
                  const x = (index / trendLength) * 100;
              ...
              {data.monthlyTrend.filter((_, index) => index % 5 === 0).map((value, index) => {
                ...
                const x = (actualIndex / trendLength) * 100;
                const y = 40 - (value / maxCalls) * 30;
                return (
                  <circle
                    key={index}
